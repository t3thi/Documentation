...
Astrid already gives the outlook that the core patch she is currently working on is a big deal. The general topic of `l10n_mode=exclude` needs to be reconsidered and cannot be fixed directly without a fundamental conceptual consideration. Christian Kuhn is currently asking himself to what extent the DataHandler should handle `l10n_mode=exclude` at all.
